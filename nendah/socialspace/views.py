...
from socialspace.models import Profile, Post, likepost, product, Category, Host, FollowersCount
...
def index(request):
    ...
    return render(request, "landing/index.html", {'posts': posts, 'user_profile': user_profile, 'user_objects': user_objects, 'suggestion_username_profile_list': suggestion_username_profile_list[:4]})
...
def profile(request):
    ...
    context = {'categories': categories, 'posts': posts, 'hosts': hosts}
...
def settings(request):
    ...
    return render(request, 'landing/profile.html', {'user_profile': user_profile})
